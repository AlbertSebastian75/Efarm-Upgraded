...
<html>
<head>
 <title><?php session_start(); echo $_SESSION['usr']; ?> | Vendor Products</title>
</head>
<body>
    <style>
		body{
...
<div class="topnav">
    ...
    <center><a href="view_vendor_product_approve.php"><button class="a7">Approved Products </button></a>
    <a href="view_vendor_product_reject.php"><button class="a7">Rejected Products </button></a>
    <a href="view_previous_admin_request.php"><button class="a7">Previous Admin Request </button></a></center>
    
    <div class="topnav-left"><h4>SUPPLIER REQUESTS</h4></div>
   <!-- <div class="topnav-right"><a href="view_vendor_product_approve.php"><button class="a7">Approved Vendor Product Request</button></a></div><br>
    <div class="topnav-right"><a href="view_vendor_product_reject.php"><button class="a7">Rejected Vendor Product Request</button></a></div><br>-->
		</tr>
...
if (mysqli_connect_error()) {
  ...
  echo "<table align=center border=1 bgcolor=LightGoldenRodYellow><tr>
    ...
    <th>ACTION</th>   
        ...
        $sql = "SELECT * from vendor_product where`status`='0'";
       ...
       if($resultCheck > 0){
        ...
        ?>
        </table><div class="topnav-left"><h4>ADMIN REQUESTS</h4></div><table>
<?php
        echo "<table align=center border=1 bgcolor=LightGoldenRodYellow><tr>
        <th>Vendor Product ID</th>
        <th>Items ID</th>
        ...
        <th>ACTION</th>  
            ...
            $sql = "SELECT * from vendor_product where`status`='10'";
            $result=mysqli_query($conn, $sql);
           $resultCheck = mysqli_num_rows($result);
           if($resultCheck > 0){
               while ($row = mysqli_fetch_assoc($result)) {
                ...
                echo "<td>".$row['req_date'] ."</td>";
                echo "
                <td><a href=../vendor/view_vendor_product.php?items_id=".$row['items_id']. "><button class=a331>View</button></a>
                <a href=admin_vendor_prod_remove.php?vendor_prod_id=".$row['vendor_prod_id']. "><button class=a331>Remove</button></a></td></tr>";
