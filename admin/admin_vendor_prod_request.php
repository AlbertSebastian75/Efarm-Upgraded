...
if (mysqli_connect_error()) {
	...
	$checkUser = "SELECT * from vendor_product where items_id='$txt2' and `status`='10'";
	$result=mysqli_query($conn, $checkUser);
	$count = mysqli_num_rows($result);

	if($count > 0){
		die("<center><h3>Item Already Added!</h3></center>");
	}


	$sql="INSERT INTO `vendor_product`(items_id,`req_date`,`status`)
	values('$txt2','$txt5',10)";
	
	if ($conn->query($sql)) {
		header('Location:vendor/view_vendor_allproduct.php');
	}
	else{
