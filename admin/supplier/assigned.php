    ...
    echo "<h4>VIEW SUPPLIERS</h4>
        ...
        <table align=center border=1 bgcolor=LightGoldenRodYellow>
           ...
           if($resultCheck > 0){
            ...
            echo "</table><table align=center border=1 bgcolor=LightGoldenRodYellow><h4>ASSIGNED ORDERS</h4>	<tr>
			<th>Sales ID</th>
			<th>User ID</th>
			<th>Item ID</th>
			<th>Quantity</th>
        ...
        $t=0;$c=0;$delivery_total=0;$delivery_count=0;
           $sql = "SELECT * from sales where supplier=$txt6 and status=2 order by date desc;";
           $result=mysqli_query($conn, $sql);
          $resultCheck = mysqli_num_rows($result);
          if($resultCheck > 0){
              while ($row = mysqli_fetch_assoc($result)) {
